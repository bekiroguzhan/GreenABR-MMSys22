...
VIDEO_BIT_RATE = [150,300, 750, 1200, 1850, 2850, 4300,6000,9000,12000]  # Kbps
...
#         print(throughput)
#         self.t_que.append(throughput) # for the use of harmonic mean 
...
#         new_state[2]= harmonic_mean(self.t_que)
       ...
       if quality >3:
           quality_reward=quality+2**(quality-3)
...
#         if energy_penalty <= -1:
#             energy_penalty=energy_penalty+(energy_penalty+1)*2
...
def main():
    ...
    ddqn_agent = DDQNAgent(alpha=0.0001, gamma=0.99, n_actions=A_DIM, epsilon=1.0,batch_size=64,input_dims=input_dims)
