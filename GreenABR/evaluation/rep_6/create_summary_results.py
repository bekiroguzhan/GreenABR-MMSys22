...
#     print(traces)
   ...
   summary_data=pd.DataFrame(index=traces,columns=['Efficiency_Avg','Efficiency_X','Efficiency_G','QoE','Bitrate','Throughput','Buffer_Size','VMAF','Rebuffer_Time','Rebuffer_Count','Smoothness_Change','Data','Energy_Avg','Energy_G','Energy_X'])
   ...
   for l in os.listdir(log_dir):
       if l.split('_', maxsplit=3)[1]==model:
           ...
           with open(log_dir + l, 'r') as f:
               ...
               summary_data.loc[trace]=[eff_avg,qoe/np.sum(energy_x)*1000,qoe/np.sum(energy_g)*1000,qoe, np.sum(bit_rate), np.sum(throughput), np.sum(buffer_size), np.sum(vmaf),np.sum(rebuffer), rebuffer_c, np.sum(smooth), np.sum(data_size),en_avg, np.sum(energy_g), np.sum(energy_x)]
